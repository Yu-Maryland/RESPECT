...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        """
        #pick = random.sample(range(len(self.DATASET)), self.opts.val_size)
        #self.dataset = Subset(self.DATASET, pick)
        print("Evaluating baseline model on evaluation dataset")
        if epoch == -1:
            cost = rollout(self.model, self.dataset, self.opts, measures=True)
            avg_cost = cost.mean()
            print('Validation overall avg_cost: {} +- {}'.format(
            avg_cost, torch.std(cost) / math.sqrt(len(cost))))

            self.bl_vals = cost.cpu().numpy()
        else:
    ...
    def eval(self, x, c):
        ...
        with torch.no_grad():
            v, _, _, _, _, _, _, _, _ = self.model(x[0], x[1], self.opts, Training=False)
