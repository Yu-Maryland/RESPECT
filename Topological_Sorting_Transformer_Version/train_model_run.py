...
def rollout(model, dataset, opts, measures=False, plot_data=False):
    ...
    def eval_model_bat(bat):
        with torch.no_grad():
            ...
            cost, _, misMatch, _, recall_accuracy, radius_mean, radius_max, recall_accuracy_max, recall_accuracy_min = model(move_to(bat[0], opts.device), bat[1], opts, Training=False, Measures=measures, Plot_Data=plot_data)
...
def train_epoch(model, optimizer, baseline, lr_scheduler, epoch, val_dataset, train_dataset_original, problem, tb_logger, opts):
    ...
    #train_dataset = Subset(train_dataset_original, random.sample(range(len(train_dataset_original)), opts.epoch_size))
    training_dataset = baseline.wrap_dataset(train_dataset_original)
