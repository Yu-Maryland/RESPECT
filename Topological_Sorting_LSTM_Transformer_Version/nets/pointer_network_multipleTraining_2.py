...
class PointerNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoder_in_0_0 = nn.Parameter(torch.FloatTensor(embedding_dim))
        self.decoder_in_0_0.data.uniform_(-std, std)

        self.decoder_in_0_1 = nn.Parameter(torch.FloatTensor(embedding_dim))
        self.decoder_in_0_1.data.uniform_(-std, std)

        self.embedding = nn.Parameter(torch.FloatTensor(self.input_dim, embedding_dim))
        self.embedding.data.uniform_(-std, std)
        """
        self.embedding_0 = nn.Parameter(torch.FloatTensor(self.input_dim, embedding_dim))
        self.embedding_0.data.uniform_(-std, std)

        self.embedding_1 = nn.Parameter(torch.FloatTensor(self.input_dim, embedding_dim))
        self.embedding_1.data.uniform_(-std, std)
        """
    ...
    def set_decode_type(self, decode_type):
        self.decoder_0.decode_type = decode_type
        self.decoder_1.decode_type = decode_type
    ...
    #def forward(self, inputs, eval_tours=None, return_pi=False, Measures=False, Plot_Data=False):
        ...
        """
        embedded_inputs_1 = self.bn1(torch.mm(
            inputs.transpose(0, 1).contiguous().view(-1, input_dim),
            self.embedding_1
        ).view(graph_size, batch_size, -1).transpose(0, 1).transpose(1, 2)).transpose(1, 2).transpose(0, 1)
        """
        ...
        #_log_p_0, pi_0, _log_p_1, pi_1 = self._inner(embedded_inputs_0, embedded_inputs_1, eval_tours)
        _log_p_0, pi_0, _log_p_1, pi_1 = self._inner(embedded_inputs, eval_tours)
        
        difference_absolute = None
        difference_pipeline = None
        ...
        #cost, mask, misMatch, _, recall_accuracy, radius_mean, radius_max, recall_accuracy_max, recall_accuracy_min = self.problem.get_costs(inputs, pi, labels, Measures, Plot_Data)
        cost_0, mask, misMatch_0, _, recall_accuracy_0_mean, recall_accuracy_0_max, recall_accuracy_0_min, radius_mean, radius_max, order_learned_0 = self.problem.get_costs(inputs, pi_0, labels, Measures, Plot_Data)
        cost_1, mask, misMatch_1, _, recall_accuracy_1_mean, recall_accuracy_1_max, recall_accuracy_1_min, radius_mean, radius_max, order_learned_1 = self.problem.get_costs(inputs, pi_1, labels, Measures, Plot_Data, P=1)

        """
        if Measures:
            difference_absolute = torch.FloatTensor([torch.nonzero(torch.sub(pi_0, pi_1)).shape[0] / pi_0.shape[0]]).cuda()
            difference_pipeline = torch.FloatTensor([torch.nonzero(torch.sub(order_learned_0, order_learned_1)).shape[0] / order_learned_0.shape[0]]).cuda()
        """
        ...
        ll_0 = self._calc_log_likelihood(_log_p_0, pi_0, mask)
        ll_1 = self._calc_log_likelihood(_log_p_1, pi_1, mask)
        if return_pi:
            #return cost, ll, pi, misMatch_y, misMatch_x, recall_accuracy, radius_mean, radius_max, recall_accuracy_max, recall_accuracy_min
            return cost_0, cost_1, ll_0, ll_1, pi_0, pi_1, misMatch_0, misMatch_1, None, recall_accuracy_0_mean, recall_accuracy_0_max, recall_accuracy_0_min, recall_accuracy_1_mean, recall_accuracy_1_max, recall_accuracy_1_min, radius_mean, radius_max, difference_absolute, difference_pipeline

        #return cost, ll, misMatch_y, misMatch_x, recall_accuracy, radius_mean, radius_max, recall_accuracy_max, recall_accuracy_min
        return cost_0, cost_1, ll_0, ll_1, misMatch_0, misMatch_1, None, recall_accuracy_0_mean, recall_accuracy_0_max, recall_accuracy_0_min, recall_accuracy_1_mean, recall_accuracy_1_max, recall_accuracy_1_min, radius_mean, radius_max, difference_absolute, difference_pipeline
    ...
    def _inner(self, inputs, eval_tours=None):
        ...
        """
        encoder_hx_0 = encoder_cx_0 = Variable(
            torch.zeros(1, inputs_0.size(1), self.encoder.hidden_dim, out=inputs_0.data.new()),
            requires_grad=False
        )

        encoder_hx_1 = encoder_cx_1 = Variable(
            torch.zeros(1, inputs_1.size(1), self.encoder.hidden_dim, out=inputs_1.data.new()),
            requires_grad=False
        )
        """
        ...
        """
        enc_h_1, (enc_h_t_1, enc_c_t_1) = self.encoder(inputs_1, (encoder_hx_1, encoder_cx_1))
        enc_h_1 = self.bn2(enc_h_1.transpose(1, 2)).transpose(1, 2)

        dec_init_state_1 = (enc_h_t_1[-1], enc_c_t_1[-1])
        """

        # repeat decoder_in_0 across batch
        decoder_input_0 = self.decoder_in_0_0.unsqueeze(0).repeat(inputs.size(1), 1)
        decoder_input_1 = self.decoder_in_0_1.unsqueeze(0).repeat(inputs.size(1), 1)

        enc_h = self.transformer_encoder(enc_h)
        enc_h = self.bn3(enc_h.transpose(1, 2)).transpose(1, 2)

        """
        enc_h_1 = self.transformer_encoder(enc_h_1)
        enc_h_1 = self.bn3(enc_h_1.transpose(1, 2)).transpose(1, 2)
        """

        (pointer_probs_0, input_idxs_0), dec_hidden_t_0 = self.decoder_0(decoder_input_0,
                                                                 inputs,
                                                                 dec_init_state,
                                                                 enc_h,
                                                                 eval_tours)

        (pointer_probs_1, input_idxs_1), dec_hidden_t_1 = self.decoder_1(decoder_input_1,
                                                                 inputs,
                                                                 dec_init_state,
                                                                 enc_h,
                                                                 eval_tours)

        return pointer_probs_0, input_idxs_0, pointer_probs_1, input_idxs_1
